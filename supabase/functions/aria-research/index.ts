...
// Z.AI GLM models (primary). OpenAI gpt-4o-mini is fallback only when Z.AI returns 429/402.
...
// Rate limit: max requests per window per key (user or IP). Reduces abuse and protects OpenAI credits.
...
/** Z.AI first; on 429/402 or error, fall back to OpenAI only. Optional delay before fallback to avoid burst rate limits. */
async function callAIWithFallback(
  ...
  systemPrompt: string,
  userPrompt: string,
  delayBeforeOpenAI = false
): Promise<{ content: string; usedFallback: "zai" | "openai" }> {
  if (zaiKey) {
    try {
      const content = await callProvider(ZAI_GATEWAY, zaiKey, zaiModel, systemPrompt, userPrompt);
      return { content, usedFallback: "zai" };
    } catch (e) {
      if (delayBeforeOpenAI && openaiKey) await new Promise((r) => setTimeout(r, 1500));
      if (openaiKey) {
        try {
          const content = await callProvider(OPENAI_GATEWAY, openaiKey, OPENAI_MODEL, systemPrompt, userPrompt);
  ...
  throw new Error("Set at least one of ZAI_API_KEY or OPENAI_API_KEY in Edge Function secrets.");
}
...
serve(async (req) => {
  ...
  if (!zaiKey && !openaiKey) {
    return new Response(
      JSON.stringify({
        error: "Set at least one of ZAI_API_KEY or OPENAI_API_KEY in Edge Function secrets. Z.AI is primary; OpenAI is fallback when rate-limited or out of credits.",
      }),
  ...
  if (!checkRateLimit(rateKey)) {
    return new Response(
      JSON.stringify({ error: "Too many research requests. Please wait a minute before trying again." }),
...
Output ONLY valid JSON with this structure:
{
  ...
  "reasoning": "Brief explanation of why this decomposition"
        ...
        if (planRes.usedFallback !== "zai") send(makeEvent("Orchestrator", "thinking", "Using OpenAI fallback."));
...
Write [1], [2], etc. for inline citations.`,
        ...
        if (researchRes.usedFallback !== "zai") send(makeEvent("Researcher", "thinking", "Using OpenAI fallback."));
...
Be rigorous. Challenge assumptions. Distinguish correlation from causation.`,
        ...
        if (analysisRes.usedFallback !== "zai") send(makeEvent("Analyst", "thinking", "Using OpenAI fallback."));
...
Make the code realistic, well-commented, and use real numbers from the research.`,
        ...
        if (coderRes.usedFallback !== "zai") send(makeEvent("Coder", "thinking", "Using OpenAI fallback."));
...
Make the report detailed (1500+ words), factual, and well-cited. Use specific numbers and dates.`,
        ...
        if (reportRes.usedFallback !== "zai") send(makeEvent("Writer", "thinking", "Using OpenAI fallback."));
...
Return 6-8 sources. Make URLs plausible (use real domains like arxiv.org, mckinsey.com, nature.com, etc.)`,
        ...
        if (msg.includes("429")) {
          ...
          send(makeEvent("Orchestrator", "error", "AI credits exhausted (Z.AI). Add Z.AI credits or set OPENAI_API_KEY in Edge Function secrets for fallback."));
        } else {
